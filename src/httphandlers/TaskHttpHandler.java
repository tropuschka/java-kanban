        ...
        switch (method) {
            case "POST":
                if (bodyArray[1].equals("task") && bodyArray.length == 2) { // Обработать параметры
                    manager.createTask(new Task(1, "name", "descr"));
                    sendText(exchange, "Task \"Name\" created");
                } else if (bodyArray[1].equals("task") && bodyArray.length == 3 && isNumber(bodyArray[2])) {
                    int taskId = Integer.parseInt(bodyArray[2]);
                    manager.updateTask(new Task(taskId, "name", "descr"));
                    sendText(exchange, "Task \"Name\" updated");
